...
Usage:
    python -m agent.main            → paper trading (default, safe)
    python -m agent.main -paper     → paper trading (explicit)
    python -m agent.main -live      → live trading (CAUTION: sets PAPER_MODE=false)
    python -m agent.main --dry-run  → full pipeline, zero DB writes, zero API calls
    python -m agent.main --skip-guard → bypass calendar_guard (for manual testing)
