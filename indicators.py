...
Performance rewrite (v2):
    All pattern math is now fully vectorized with NumPy arrays.
    ...
    Key vectorization strategy:
        ...
        2. Each detector receives the full matrix and returns a boolean
           mask of which symbols matched, plus per-symbol scalar results.
        3. CandlePattern objects are only instantiated for matched symbols
           (not all 335).

    Single-symbol detect_patterns() still works for filter_engine.py
    per-symbol calls — it slices a 1-row matrix and runs the same path.
...
Design decisions (unchanged from v1):
    - Takes last LOOKBACK candles from HistoryCache for each symbol.
    - Appends today's live PriceRow as the most recent candle.
    - Returns list[CandlePattern] per symbol.
...
Pattern tiers:
    Tier 1 — Highest reliability (use for entry signals):
        ...
        Morning Star, Evening Star, Doji at Support/Resistance
...
def _volume_ratio(vol_matrix: np.ndarray) -> np.ndarray:
    """
    vol_matrix shape: (n_symbols, LOOKBACK)
...
def _is_downtrend(close_matrix: np.ndarray, periods: int = 3) -> np.ndarray:
    """
    True for each symbol where the last `periods` HISTORICAL closes are declining.
    close_matrix[:, -1] is today — trend uses closes[:, -periods-1:-1].
    """
    if close_matrix.shape[1] < periods + 1:
        return np.zeros(close_matrix.shape[0], dtype=bool)
    window = close_matrix[:, -(periods + 1):-1]   # shape (n, periods)
    diffs  = np.diff(window, axis=1)               # shape (n, periods-1)
...
def _build_matrices(
    ...
    Returns:
        ...
        C_full         np.ndarray (n_valid, max_hist)  padded with NaN on left
    """
    n_hist = LOOKBACK - 1   # historical candles excluding today
...
def _detect_three_black_crows(symbols, O, H, L, C, V, C_full):
    ...
    match = (
        ...
        (c1 > c2) & (c2 > c3) &
        (o2 >= c1 * 0.97) & (o2 <= c1 * 1.03) &
        (o3 >= c2 * 0.97) & (o3 <= c2 * 1.03)
    )
    vol_confirmed = _volume_ratio(V) >= VOLUME_SURGE_RATIO
...
def detect_all_patterns(
    ...
    Strategy:
        1. Build O/H/L/C/V matrices (n_symbols x LOOKBACK) in one shot.
        2. Run each detector over the full matrix — returns only matched rows.
        3. Assemble per-symbol pattern lists from the index-tagged results.
...
def write_patterns_to_db(all_patterns: dict[str, list[CandlePattern]]) -> int:
    """Upsert pattern metadata to Neon candle_patterns table."""
    try:
        from db import upsert_row
    except ImportError:
        logger.warning("write_patterns_to_db: db not available")
        return 0
    ...
    for sym, patterns in all_patterns.items():
        for p in patterns:
            ...
            try:
                upsert_row(
                    "candle_patterns",
                    {"pattern_name": p.name, "type": p.signal,
                     "tier": str(p.tier), "reliability": str(p.confidence),
                     "notes": p.description},
    ...
    logger.info("write_patterns_to_db: %d rows to Neon", written)
    return written
