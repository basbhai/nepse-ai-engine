...
import modules.trading_core as trading_core
...
# SANDBOX_MODE lives in trading_core — set via trading_core.SANDBOX_MODE in main()
# _circuit_breakers is the shared mutable dict from trading_core
from modules.trading_core import _circuit_breakers


# ═══════════════════════════════════════════════════════════════════════════
# SHARED LOGIC — imported from trading_core
# ═══════════════════════════════════════════════════════════════════════════

from modules.trading_core import (
    nst_now, nst_today, hold_days,
    calc_buy_fees, calc_sell_fees,
    get_user, is_approved, get_all_approved_ids, register_user, approve_user, deny_user,
    get_paper_capital, get_paper_position, get_all_open_positions, count_open_positions, lookup_symbols,
    reset_user_paper_data,
    execute_buy, execute_sell,
    mode_label, circuit_label, fmt_npr, win_rate_str, sandbox_label, market_gate_message,
)
...
async def cmd_claude(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Admin-only: run a one-off Claude prompt with all local MCP servers (nepse-engine,
    atrad-scraper, docs, email) + curated Playwright + claude_agent_reports/ Read/Write/Edit."""
    if not is_admin(update.effective_user.id):
        await update.message.reply_text("🚫 Admin only command.")
        return
...
def main():
    ...
    trading_core.SANDBOX_MODE = args.sandbox
