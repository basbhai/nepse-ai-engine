...
def check_tabs_needing_work(tab_schemas: dict) -> dict:
    """
    DRY RUN — scan all tabs and report what needs to be done WITHOUT making any changes.
    Zero writes. Minimal reads (one API call per tab to fetch headers).

    Returns:
        Dict of {tab_name: {"status": str, "missing_cols": list, "needs_resize": bool}}

    Status values:
        "OK"          — tab exists, all columns present, grid is large enough
        "MISSING_TAB" — tab doesn't exist at all
        "NEEDS_COLS"  — tab exists but missing one or more columns
        "NEEDS_RESIZE"— tab exists, columns OK, but grid is too narrow for column count
    """
    results = {}
    try:
        sheet = _get_sheet()
        existing_tab_names = {ws.title: ws for ws in sheet.worksheets()}
    except Exception as e:
        log.error(f"check_tabs_needing_work: could not list worksheets — {e}")
        return {}
    ...
    for tab_name, columns in tab_schemas.items():
        try:
            ...
            ws = existing_tab_names[tab_name]
            existing_headers = ws.row_values(1)
            missing = [c for c in columns if c not in existing_headers]
            # Check if grid is wide enough for all required columns
            # gspread worksheet object has .col_count
            try:
                grid_cols = ws.col_count
            except Exception:
                grid_cols = len(existing_headers)
            needs_resize = grid_cols < len(columns)
...
def initialize_all_tabs(only_needed: bool = True) -> dict:
    """
    Ensure ALL system tabs exist with correct columns.

    Args:
        only_needed: If True (default), first does a dry-run check and ONLY
                     creates/migrates tabs that are actually missing or incomplete.
                     This saves API quota dramatically on repeated runs.
                     If False, calls ensure_tab on every tab unconditionally
                     (original behaviour — use only when you suspect the check itself
                     is wrong).

    Returns:
        Dict of {tab_name: status_string}

    Usage:
        python sheets.py init          — smart mode (only acts on what needs work)
        python sheets.py init --force  — brute-force mode (checks every tab)
    ...
    # ── Smart mode: dry-run check first, then only act on what needs work ──
    if only_needed:
        log.info("initialize_all_tabs: running dry-run check first...")
        check = check_tabs_needing_work(TAB_SCHEMAS)

        # Separate tabs by what they need
        tabs_ok         = [t for t, r in check.items() if r["status"] == "OK"]
        tabs_missing    = [t for t, r in check.items() if r["status"] == "MISSING_TAB"]
        tabs_need_cols  = [t for t, r in check.items() if r["status"] == "NEEDS_COLS"]
        tabs_need_resize= [t for t, r in check.items() if r["status"] == "NEEDS_RESIZE"]
        tabs_error      = [t for t, r in check.items() if r["status"].startswith("ERROR")]

        log.info(
            f"Dry-run summary — OK: {len(tabs_ok)}, "
            f"MISSING: {len(tabs_missing)}, "
            f"NEEDS_COLS: {len(tabs_need_cols)}, "
            f"NEEDS_RESIZE: {len(tabs_need_resize)}, "
            f"ERROR: {len(tabs_error)}"
        )

        results = {}

        # Mark OK tabs immediately — zero additional API calls
        for tab_name in tabs_ok:
            results[tab_name] = "✓ Already OK (skipped)"

        # Mark error tabs
        for tab_name in tabs_error:
            results[tab_name] = f"✗ Check error: {check[tab_name]['status']}"

        # Only act on tabs that actually need work
        tabs_to_fix = tabs_missing + tabs_need_cols + tabs_need_resize
        if not tabs_to_fix:
            log.info("initialize_all_tabs: all tabs already up to date — nothing to do.")
            return results

        log.info(f"initialize_all_tabs: acting on {len(tabs_to_fix)} tab(s): {tabs_to_fix}")

        for tab_name in tabs_to_fix:
            try:
                # Pre-resize grid if needed to avoid "exceeds grid limits" error
                if check[tab_name]["needs_resize"] or tab_name in tabs_need_cols:
                    try:
                        ws = _get_sheet().worksheet(tab_name)
                        required_cols = len(TAB_SCHEMAS[tab_name]) + 5
                        if ws.col_count < required_cols:
                            ws.resize(rows=ws.row_count, cols=required_cols)
                            log.info(f"  Pre-resized {tab_name} to {required_cols} cols")
                    except Exception:
                        pass  # Tab doesn't exist yet — create_tab will handle sizing

                success = ensure_tab(tab_name, TAB_SCHEMAS[tab_name])
                status  = check[tab_name]["status"]
                missing = check[tab_name]["missing_cols"]
                if success:
                    if status == "MISSING_TAB":
                        results[tab_name] = f"✓ Created ({len(TAB_SCHEMAS[tab_name])} cols)"
                    elif status == "NEEDS_COLS":
                        results[tab_name] = f"✓ Added columns: {missing}"
                    elif status == "NEEDS_RESIZE":
                        results[tab_name] = "✓ Resized grid"
                    else:
                        results[tab_name] = "✓ Fixed"
                else:
                    results[tab_name] = f"✗ Failed (status was: {status})"

                time.sleep(1)  # Avoid API rate limits between writes
    ...
    # ── Force mode: original behaviour — check every tab unconditionally ──
    results = {}
    for tab_name, columns in TAB_SCHEMAS.items():
        try:
            # Pre-resize to avoid grid limits errors
            try:
                ws = _get_sheet().worksheet(tab_name)
                required_cols = len(columns) + 5
                if ws.col_count < required_cols:
                    ws.resize(rows=ws.row_count, cols=required_cols)
                    log.info(f"  Pre-resized {tab_name} grid to {required_cols} cols")
            except Exception:
...
if __name__ == "__main__":
    ...
    # ── python sheets.py check ──────────────────────────────────────────────
    # Dry-run: reads headers from every tab, reports what needs work.
    # Zero writes. Safe to run any time.
    if cmd == "check":
        print("\n" + "=" * 55)
        print("NEPSE AI — sheets.py DRY-RUN CHECK (no changes made)")
        print("=" * 55 + "\n")
        print("Reading tab headers from Google Sheets...")

        # initialize_all_tabs in smart mode does its own dry-run check first
        # and skips OK tabs — no writes happen unless something is broken.
        # For a pure zero-write check we call initialize_all_tabs(only_needed=True)
        # and look at the "Already OK" results vs anything that got fixed.
        # If you want TRULY zero writes: read the logs — any tab logged as
        # "✓ Already OK (skipped)" had zero API writes.
        results = initialize_all_tabs(only_needed=True)

        ok     = [(t, s) for t, s in results.items() if "Already OK" in s]
        fixed  = [(t, s) for t, s in results.items() if s.startswith("✓") and "Already OK" not in s]
        failed = [(t, s) for t, s in results.items() if s.startswith("✗")]

        print(f"\n{'Tab':<30} Status")
        print("-" * 55)
        for tab, status in results.items():
            print(f"  {tab:<28} {status}")
        print("-" * 55)
        print(f"\n  Already OK (zero writes) : {len(ok)}")
        print(f"  Fixed/Created            : {len(fixed)}")
        print(f"  Errors                   : {len(failed)}")
        print()

    # ── python sheets.py init ───────────────────────────────────────────────
    # SMART mode: dry-run first, only act on tabs that need work.
    # Dramatically fewer API calls than brute-force.
    elif cmd == "init":
        force = "--force" in flags
        print("\n" + "=" * 55)
        if force:
            print("NEPSE AI — Creating / Verifying Tabs  [FORCE MODE]")
        else:
            print("NEPSE AI — Creating / Verifying Tabs  [SMART MODE]")
            print("(Only acts on tabs that are missing or incomplete)")
            print("Use 'python sheets.py init --force' to check every tab")
        print("=" * 55 + "\n")

        results = initialize_all_tabs(only_needed=not force)
        ...
        if fixed:
            print("── Changes made ──────────────────────────────────")
            for tab, status in fixed:
                print(f"  {tab:<28} {status}")

        if failed:
            print("\n── Errors ────────────────────────────────────────")
            for tab, status in failed:
                print(f"  {tab:<28} {status}")

        print(f"\n  Tabs already OK  : {len(ok)} (no API calls used)")
        print(f"  Tabs fixed       : {len(fixed)}")
        print(f"  Errors           : {len(failed)}")
        print("\nDone.\n")

    # ── python sheets.py  (no args) ─────────────────────────────────────────
    else:
        print("\n" + "=" * 55)
        print("NEPSE AI — sheets.py Health Check")
        print("=" * 55)
        print("\nCommands:")
        print("  python sheets.py              — connection health check (this screen)")
        print("  python sheets.py check        — dry-run: show what needs fixing (no writes)")
        print("  python sheets.py init         — smart init: only fix what's broken")
        print("  python sheets.py init --force — brute-force: check every tab (more API calls)")
        print()
